...
func TestNodeInfo(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			modFn: func() *NodeInfo {
				return NewNodeInfo(node, nil, nil, testPodInfos(pods, false)...)
			},
		...
		},
		{
			...
			modFn: func() *NodeInfo {
				return NewNodeInfo(node, slices, nil, testPodInfos(pods, true)...)
			},
			wantSchedNodeInfo:       schedulerNodeInfo,
		...
		},
		{
			...
			modFn: func() *NodeInfo {
				return NewNodeInfo(node, nil, &storagev1.CSINode{ObjectMeta: v1.ObjectMeta{Name: nodeName}}, testPodInfos(pods, false)...)
			},
			wantSchedNodeInfo: schedulerNodeInfo,
			wantPods:          testPodInfos(pods, false),
		...
		},
		{
			...
			modFn: func() *NodeInfo {
				result := NewNodeInfo(nil, nil, nil)
				result.SetNode(node)
		...
		},
		{
			...
			modFn: func() *NodeInfo {
				result := NewNodeInfo(node, slices, nil, testPodInfos(pods, true)...)
...
func TestNodeInfoResourceClaims(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			nodeInfo:   NewNodeInfo(node, nil, nil, testPodInfos(pods, false)...),
			wantClaims: nil,
		},
		{
			testName: "pods with claims, shared claims are not repeated",
			nodeInfo: NewNodeInfo(node, nil, nil, testPodInfos(pods, true)...),
			wantClaims: []*resourceapi.ResourceClaim{
