...
func TestCheckCapacityBatchMarksMaterializationErrorsFailed(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, status.ScaleUpNotTried, exported.Result)
...
func TestCheckCapacityBatchTreatsSchedulingErrorAsInternalError(t *testing.T) {
	...
	require.NoError(t, baseSnapshot.SetClusterState(nil, nil, nil, nil))
...
func TestCheckCapacityBatchTreatsAddClaimsErrorAsInternalError(t *testing.T) {
	...
	require.NoError(t, snapshot.SetClusterState(nil, nil, nil, nil))
...
func TestMaterializedWorkloadSchedulesInDraSnapshot(t *testing.T) {
	...
	require.NoError(t, clusterSnapshot.SetClusterState([]*corev1.Node{node}, nil, draState, csisnapshot.NewEmptySnapshot()))
